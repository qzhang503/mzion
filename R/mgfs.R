                       ...
                       quant = "none", digits = 4L) 
{
  ...
  args_except <- c("out_path", "n_dia_ms2s")
...
#' @inheritParams matchMS
proc_mzml <- function (file, raw_id, filepath, topn_ms2ions = 150L, 
                       min_ms1_charge = 2L, max_ms1_charge = 4L, 
                       min_scan_num = 1L, max_scan_num = .Machine$integer.max, 
                       min_ret_time = 0, max_ret_time = Inf, 
                       min_mass = 200L, max_mass = 4500L, 
                       ...
                       digits = 4L) 
{
  df <- read_mzml(file, filepath = filepath, 
                  topn_ms2ions = topn_ms2ions, 
                  tmt_reporter_lower = tmt_reporter_lower, 
                  tmt_reporter_upper = tmt_reporter_upper, 
                  exclude_reporter_region = exclude_reporter_region, 
  ...
  is_dia <- attr(df, "is_dia", exact = TRUE)
  
  if (is_dia && topn_ms2ions <= 500L) {
    topn_ms2ions <- 500L
    warning("Reset the maximum number of DIA-MS2 features to 500.")
  }
  ...
  post_readmgf(df, raw_id = raw_id, mgf_path = filepath)
}
...
#' @param filepath A file path of MGF.
#' @inheritParams matchMS
read_mzml <- function (xml_file, filepath, topn_ms2ions = 150L, 
                       tmt_reporter_lower = 126.1, tmt_reporter_upper = 135.2, 
                       exclude_reporter_region = FALSE, 
                       ppm_ms1 = 10L, ppm_ms2 = 10L, 
                       min_mass = 200L, max_mass = 4500L, 
                       ...
                       digits = 4L)
{
  ...
  for (i in rng) {
    ...
    if (xml2::xml_attr(xc[[idx_mslev]], "value") == "2") {
      ...
      # maxn_dia_per_da <- 10L
      # iso_lwr <- xml2::xml_attr(isolationWindowc[[idx_lwrmz]], "value")
      # maxn_dia_precurs <- floor(as.numeric(iso_lwr) * 2 * maxn_dia_per_da)
      ...
      # better let user define? 
      #   "charge state" may be missing in early scans of MSConvert DDA 
  ...
  for (i in rng) {
    ...
    if (as.integer(xml2::xml_attr(xc[[idx_mslev]], "value")) == 1) {
      ...
      # for later remove the assumption of a fixed resolution with Astral
  ...
  if (is_dia) {
    df <- proc_dia(spec = spec, raw_file = raw_file, filepath = filepath, 
                   is_demux = is_demux, 
                   ...
                   idx_bin_2 = idx_bin_2, 
                   maxn_dia_precurs = maxn_dia_precurs, n_dia_ms2s = n_dia_ms2s, 
  ...
  else if (maxn_mdda_precurs) {
    df <- proc_mdda(spec = spec, raw_file = raw_file, idx_sc = idx_sc, 
                    idx_osc = idx_osc, idx_mslev = idx_mslev, idx_title = idx_title, 
    ...
    df <- proc_dda(spec = spec, raw_file = raw_file, idx_sc = idx_sc, 
                   idx_osc = idx_osc, idx_mslev = idx_mslev, idx_title = idx_title, 
                   idx_scanList_2 = idx_scanList_2, idx_rt_2 = idx_rt_2, 
                   idx_scan_start_2 = idx_scan_start_2, 
                   idx_precursor_2 = idx_precursor_2, 
...
proc_mdda <- function (spec, raw_file, idx_sc = 3L, idx_osc = 3L, idx_mslev = 2L, 
                       ...
                       min_mass = 200L, max_mass = 4500,
                       min_ms2mass = 115L, max_ms2mass = 4500L, 
                       min_scan_num = 1L, max_scan_num = .Machine$integer.max, 
                       min_ret_time = 0, max_ret_time = Inf, maxn_precurs = 5L, 
                       min_ms1_charge = 2L, max_ms1_charge = 4L, 
                       ...
                       grad_isotope = 1.6, fct_iso2 = 3.0, use_defpeaks = FALSE)
{
  ...
  for (i in 1:len) {
    ...
    if (ms_lev == 2L) {
      ...
      if (length(msData) == 2L) {
        ...
        if (deisotope_ms2) {
          mic <- find_ms1stat(moverzs = msx_xs, msxints = msx_ys, 
  ...
  #  better filter by retention times
  ...
  for (i in 1:(len <- length(ms1_stas))) {
  ...
  if (deisotope_ms2) {
    ...
    if (nrow(df2)) {
      ...
      ans2 <- mapply(find_ms1byms2, 
                     ...
                     charges = df2$msx_charges, center = df2$iso_ctr, 
  ...
  ans_rts <- reset_rettimes(ret_times = df$ret_time, min_ret_time = min_ret_time, 
                            max_ret_time = max_ret_time)
  min_ret_time <- ans_rts$min_ret_time
  max_ret_time <- ans_rts$max_ret_time
  ...
  if (maxn_precurs >= 1L) {
    ...
    df$ms1_int <- mapply(function (x, y) x[y], df$ms1_int, oks, 
                         SIMPLIFY = FALSE, USE.NAMES = FALSE)
    # ms1_mass may be numeric(0) following the above filtration
...
#' @param gap The allowance in tracing MS1 and MS2.
#' @inheritParams proc_mdda
#' @inheritParams matchMS
proc_dia <- function (spec, raw_file, filepath, is_demux = FALSE, 
                      ...
                      min_scan_num = 1L, max_scan_num = .Machine$integer.max, 
                      min_ret_time = 0, max_ret_time = Inf, 
                      maxn_dia_precurs = 500L, n_dia_ms2s = 0L, 
                      min_ms1_charge = 2L, max_ms1_charge = 4L, 
                      ppm_ms1 = 10L, ppm_ms2 = 10L, 
                      min_mass = 200L, max_mass = 4500L,
                      min_ms2mass = 115L, max_ms2mass = 4500L, 
                      ppm_ms1_deisotope = 8L, ppm_ms2_deisotope = 8L, 
                      deisotope_ms2 = TRUE, max_ms2_charge = 3L, gap = 1L, 
                      grad_isotope = 1.6, fct_iso2 = 3.0, topn_ms2ions = 150L, 
                      quant = "none", 
                      tmt_reporter_lower = 126.1, tmt_reporter_upper = 135.2, 
                      exclude_reporter_region = FALSE)
{
  df <- read_dia(spec = spec, raw_file = raw_file, is_demux = is_demux, 
                 idx_sc = idx_sc, idx_osc = idx_osc, idx_mslev = idx_mslev, 
                 idx_title = idx_title, idx_scanList_1 = idx_scanList_1, 
                 idx_scanList_2 = idx_scanList_2, idx_rt_1 = idx_rt_1, 
                 idx_rt_2 = idx_rt_2, idx_scan_start_1 = idx_scan_start_1, 
                 ...
                 grad_isotope = grad_isotope, 
                 fct_iso2 = fct_iso2, 
                 topn_ms2ions = 500L, # DIA deisotoping
                 quant = quant, 
                 tmt_reporter_lower = tmt_reporter_lower, 
                 tmt_reporter_upper = tmt_reporter_upper, 
                 exclude_reporter_region = exclude_reporter_region)
  ...
  ### debugs
  # df <- qs::qread(file.path(filepath, "df_wide_0.rds"))
  qs::qsave(df, file.path(filepath, "df_wide_0.rds"), preset = "fast")
  ###

  df <- get_dia_ms1s(df)
  
  ### debugs
  qs::qsave(df, file.path(filepath, "df_wide_dia.rds"), preset = "fast")
  # df <- qs::qread(file.path(out_path, "df_wide_dia.rds"))
  ###
  ...
  n_cores <- min(parallel::detectCores(), length(dfs), 13L)
  cl <- parallel::makeCluster(getOption("cl.cores", n_cores))
  dfs <- parallel::clusterApply(cl, dfs, strace_dia, 
                                min_mass = min_mass, 
                                min_ms2mass = min_ms2mass, 
  ...
  ### debugs
  qs::qsave(df, file.path(filepath, "df_dia.rds"), preset = "fast")
  ###
...
#' Reads DIA mzML.
#' 
#' @inheritParams proc_dia
read_dia <- function (spec, raw_file, is_demux = FALSE, idx_sc = 5L, idx_osc = 3L, 
                      idx_mslev = 2L, idx_title = 10L, idx_scanList_1 = 11L, 
                      idx_scanList_2 = 11L, idx_rt_1 = 2L, idx_rt_2 = 2L, 
                      idx_scan_start_1 = 1L, idx_scan_start_2 = 1L, 
                      idx_precursor_2 = 12L, idx_isolationWindow = 1L, 
                      ...
                      ppm_ms1_deisotope = 8L, ppm_ms2_deisotope = 8L, 
                      deisotope_ms2 = TRUE, max_ms2_charge = 3L, gap = 1L, 
                      grad_isotope = 1.6, fct_iso2 = 3.0, topn_ms2ions = 500L, 
                      quant = "none", 
                      tmt_reporter_lower = 126.1, tmt_reporter_upper = 135.2, 
                      exclude_reporter_region = FALSE)
{
...
#' Helper for MS1 and MS2 deconvolution.
#'
...
#' @param tr_ms2 Logical; to trace MS2 or not.
#' @inheritParams matchMS
strace_dia <- function (df, min_mass = 200L, min_ms2mass = 115L, step1 = 1E-5, 
                        step2 = 1E-5, n_dia_ms2s = 4L)
{
                      ...
                      exclude_reporter_region = FALSE)
{
  ...
  # Low priority: no filtration by scan numbers; better by retention times
  rows <- !is.na(ms1_charges) | # "" -> NA
    ms1_charges >= min_ms1_charge | ms1_charges <= max_ms1_charge | 
    ret_times >= min_ret_time | ret_times <= max_ret_time | 
    ms1_masses >= min_mass | ms1_masses <= max_mass
...
#' @param xs Vectors of moverzs
#' @param ys Vectors of intensities
calc_ms1xys <- function (xout, yout)
{
...
#' @inheritParams matchMS
find_mdda_mms1s <- function (df1, df2, 
                             ...
                             use_defpeaks = FALSE, width = 2.01, step = ppm/1e6)
{
  ...
  ansx1 <- ansy1 <- vector("list", len1 <- length(df1$msx_moverzs))
